    ...
    <>
      ...
      >
        ...
        sx={{
          ...
          <Alerts /><Box className={styles.aboutTeamContainerBox}>
            {teams && teams.length > 0 && teams.map((team: any, i: number, row: any) => (
                <><Box className={styles.aboutTeamBox}>
                    <Typography variant="h5" fontWeight="600" component="div" className={styles.aboutTeamBoxTitle}>
              {team.name}
            </Typography>
              <Grid container spacing={4} className={styles.aboutTeamGrid}>
                    {team.members && team.members.length > 0 && team.members.map(
                        (member: any) => (
                            <Grid direction="column" display="flex" alignItems="center" flexDirection="column" className={styles.aboutTeamBoxMember}>
                                <Avatar
                                  alt={member.user.firstname + " " + member.user.lastname}
                                  src={BACKEND_BASE_URL + member.picture}
                                  className={styles.aboutTeamAvatar}
                                />
                                <Typography variant="body1" fontWeight="600">{member.user.firstname}{" "}{member.user.lastname}</Typography>
                                <Typography variant="body2" color="textSecondary">{member.role.name}</Typography>
                            </Grid>
                        )
                    )}</Grid>


                </Box>{i + 1 < row.length && <Divider />}</>))}</Box>
        </Container>
              </Grid>
      </Box>
    </>
