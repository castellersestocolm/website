...
function EventBox({ content, asList = false }: any) {
  if (asList) {
    return <Box>{content}</Box>;
  }

  return (
    <Grid className={styles.eventGrid} display="flex" alignItems="stretch">
...
function EventsBox({ content, asCard = false }: any) {
        ...
        <>
          ...
          >
            ...
            >
                  ...
                  <>
                    ...
                    {castles && castles.length > 0 && (
                      <Box className={styles.eventCastlesBox}>
                        ...
                        {castles.map((castle: any, i: number, row: any) => {
                          return (
                            <Typography
                              variant="body2"
                              color="textSecondary"
                          ...
                          <Box className={styles.eventRegistrationsBox}>
                            {eventUsers.map((eventUser: any) => {
                              ...
                              return (
                                <Box
                                  key={eventUser.id}
                                  className={styles.eventRegistrationBox}
                                  style={{
                                    color: registration
                                      ? registration.status ===
              ...
              />
              {((!isPast && family && event.require_signup) ||
                (isPast &&
                  event.google_album &&
                  (event.google_album.photos_album ||
                    event.google_album.drive_album)) ||
                (user && castlesPublished && castlesPublished.length > 0)) && (
                <Stack
                  direction={
                    calendarMatchesSm
                      ? calendarMatchesLg
                    ...
                    !event.google_album.drive_album && (
                      <Button
                        ...
                        href={
                          ...
                          event.google_album.photos_album &&
                          GOOGLE_PHOTOS_URL +
                            event.google_album.photos_album.external_id
                        }
                      >
                        {t(
                  ...
                  {isPast &&
                    ...
                    event.google_album.drive_album && (
                      <Button
                        ...
                        disableElevation
                        disabled={
                          !event.google_album || !event.google_album.drive_album
                        }
                        href={
                          event.google_album &&
                          event.google_album.drive_album &&
                          GOOGLE_DRIVE_URL +
                            event.google_album.drive_album.external_id
                        }
                      >
                        {t(
                  ...
                  {eventUsers &&
                    ...
                    castlesPublished.length > 0 && (
                      <Button
                        ...
                        style={{ width: "auto" }}
                        color={
                          eventsCastlesOpen[event.id] ? "secondary" : "primary"
                        }
                        disableElevation
                        onClick={(e) => {
                          e.stopPropagation();
          ...
          {eventUsers && eventUsers.length > 0 && event.require_signup && (
            <Collapse
              in={eventsRegistrationsOpen[event.id]}
              timeout="auto"
              unmountOnExit
                  ...
                  {castlesPublished.map((castle: any) => (
                    <Tab
                      label={
                        ...
                        >
                          {castle.name}
                          {!castle.is_published && <IconVisibilityOff />}
                        </Typography>
                      }
                    />
                  ))}
          ...
          {!isPast && event.location !== null && (
            <Collapse in={eventsMapOpen[event.id]} timeout="auto" unmountOnExit>
              <Divider />
          ...
          {asList && i + 1 < row.length && <Divider />}
        </>
      );

      return <EventBox content={eventContent} asList={asList} />;
    });
