...
// Reusable full membership registration form extracted from /app/[locale]/join/page.tsx
// Handles adult1 (required), optional adult2, optional children, consents, password setup, QR payment generation.
...
interface JoinFormProps {
  onSuccess?: () => void;
  locale?: string; // optional locale for navigation, fallback handled by pages
}

export function JoinForm({ onSuccess, locale }: JoinFormProps) {
  ...
  const onSubmit = async (data: RegistrationFormData) => {
    ...
    try {
      ...
      else setApiError(t("error.generic"));
    }
  };
          ...
          <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
            ...
            {errors.adult2 && typeof errors.adult2.message === "string" && (
              <div className="col-span-2">
                <p className="text-sm text-red-500">{errors.adult2.message}</p>
              </div>
            )}
          </div>
        )}
        ...
        {showChildren && (
          <div className="space-y-4">
            {fields.map((field, index) => (
              ...
              >
                <div className="mb-3 flex items-center justify-between">
                  ...
                  >
                    <Minus size={16} /> {t("form.remove_child")}
                  </button>
                </div>
            ...
            {fields.length < 10 && (
              ...
              >
                <Plus size={20} /> {t("form.add_child")}
              </button>
            )}
            {fields.length >= 10 && (
              <p className="text-sm text-gray-500">{t("form.max_children")}</p>
            )}
          </div>
        )}
      </div>
