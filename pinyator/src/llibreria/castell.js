...
document.onkeydown = function(event) 
{
     if (!event)
	 {
          event = window.KeyboardEvent;
     }
	 var code = event.key;
	 if ((code == "Delete") && !TeFocusAlturaExtra)
	{
		Esborra();
...
function ShowPopupCasella()
{
	if (canShowPopup)
	{
		var popup = document.getElementById("floatdiv");	
		popup.classList.add("show");
		//Recuperem info i la posem al popup
		if (mySel[0] != null)
		{
			document.getElementById("popupheaderCaption").innerHTML = mySel[0].malnom.toUpperCase();
			document.getElementById("popupheaderCaption").href = "Casteller_Fitxa.php?id=".concat(mySel[0].castellerid);
...
function ClearPopupCasella()
{
	var popup = document.getElementById("floatdiv");
	document.getElementById("popupheaderCaption").innerHTML = "";
	document.getElementById("popupheaderCaption").href = "";
	document.getElementById("popupAlturaExtra").value = 0;
}
...
function MostraPopup()
{
	...
	var hdr = document.getElementById('floatdivheader');
	...
	if (canShowPopup)
	{
		...
		hdr.addEventListener('touchmove', touchmoveEvent);
		/* record the position of the touch
		when released using touchend event.
		This will be the drop position. */  
		hdr.addEventListener('touchend', touchendEvent);
	}
	else
	{ 
		HidePopupCasella();
		hdr.removeEventListener('touchmove', touchmoveEvent);
		hdr.removeEventListener('touchend', touchendEvent);
	}
}
...
function myDown(e)
{
	...
	for (var i = l-1; i >= 0; i--) 
	{
		if (EsPenstanyaActual(boxes[i].pestanya))
		{
			...
			var imageData = gctx.getImageData(mx, my, 1, 1, { willReadFrequently: true });
...
function Save(canviFitxa) 
{
	...
	{		
		...
		xmlhttp.open("GET", "Castell_Posicio_Desa.php?obj=" + getPosicio(mySel, canviFitxa), true);
		xmlhttp.send();
	}
}

function getPosicio(obj, canviFitxa)
{
	var arry = [];
...
function SetCasteller(elem)
{
	...
	if(casteller.id > 0)
	{
		...
		casteller.altura = ToInt(elem.getAttribute("altura"));
		casteller.peu = ToInt(elem.getAttribute("peu"));
		casteller.lesionat = ToInt(elem.getAttribute("lesionat"));
...
function CanviAlturaExtra(t)
{
	if (mySel[0] != null)
	{
		if(t == '') t = 0;
		mySel[0].alturaExtra = ToInt(t);
		casteller.alturaExtra = ToInt(t);
...
function ModificaCasellesLinkades(id, objCasteller, castellId)
{
	var trobat = false;
	var i =0;
	while (!trobat && (i < boxes.length))
...
function CanviPeu()
{
	if (mySel[0] != null)
	{
		mySel[0].peu = switchInt(mySel[0].peu);
		casteller.peu = ToInt(mySel[0].peu);
		ModificaCasellesLinkades(mySel[0].id, casteller, mySel[0].castellId);	
		...
		if (parentNode != null)
		{
			if (mySel[0].peu == 0)
			{
				...
				newNode.id = "peu"+mySel[0].castellerid;
				newNode.src = "icons/peu_vermell.png";  
				node = document.getElementById("progress"+mySel[0].castellerid);	
				parentNode.insertBefore(newNode, node);	
			}
			else
				document.getElementById("peu"+mySel[0].castellerid).remove();

			parentNode.setAttribute("peu", mySel[0].peu);
		}
		Save(1);
		invalidate();
	}
...
function CanviLesionat()
{
	if (mySel[0] != null)
	{
		mySel[0].lesionat = switchInt(mySel[0].lesionat);
		casteller.lesionat = ToInt(mySel[0].lesionat);
		ModificaCasellesLinkades(mySel[0].id, casteller, mySel[0].castellId);
		parentNode = document.getElementById(mySel[0].castellerid);
		if (parentNode != null)
		{
			if (mySel[0].lesionat == 1)
			{
				newNode = document.createElement("img");
				newNode.id = "lesionat"+mySel[0].castellerid;
				newNode.src = "icons/lesionat.png";  
				node = document.getElementById("progress"+mySel[0].castellerid);	
				parentNode.insertBefore(newNode, node);	
			}
			else
				node = document.getElementById("lesionat"+mySel[0].castellerid).remove();

			parentNode.setAttribute("lesionat", mySel[0].lesionat);
		}
		Save(1);
		invalidate();
	}
