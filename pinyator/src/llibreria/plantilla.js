	...
	if (item != null)
	{
		...
		xmlhttp.onreadystatechange = function() 
		{
			if (this.readyState == 4 && this.status == 200) 
			{
				...
				{
					item.id=this.responseText.replace(/^\s+/,"");
				}
				else
				{
...
function myDblClick(e) 
{
	...
	else
	{
		...
		var width = ToInt(document.getElementById("posicioW").value);
		var height = ToInt(document.getElementById("posicioH").value);
		var angle = ToInt(document.getElementById("posicioangle").value);
...
function SetSeguents()
{
    ...
    for (var i = 1; i < l; i++) 
	{
        mySel[i-1].seguent = ToInt(mySel[i].id);
    }
	Save();
}
...
function Alinia()
{
	...
	if(l > 1)
	{
		...
		for (var i = 1; i < l; i++) 
		{
			mySel[i].x = mySel[0].x + (i * ToInt(xDif/(l-1)));
			mySel[i].y = mySel[0].y + (i * ToInt(yDif/(l-1)));
		}
