...
function initCanvas(plantillaId, esReadOnly) 
{
  ...
  {
    stylePaddingLeft = ToInt(document.defaultView.getComputedStyle(canvas, null)['paddingLeft'], 10)      || 0;
    stylePaddingTop  = ToInt(document.defaultView.getComputedStyle(canvas, null)['paddingTop'], 10)       || 0;
    styleBorderLeft  = ToInt(document.defaultView.getComputedStyle(canvas, null)['borderLeftWidth'], 10)  || 0;
    styleBorderTop   = ToInt(document.defaultView.getComputedStyle(canvas, null)['borderTopWidth'], 10)   || 0;
  }
...
function draw() 
{
	if (canvasValid == false) 
	{
		...
		for (var i = 0; i < l; i++) 
		{
			...
			if (IsInmySelById(boxes[i].id, boxes[i].castellId))
			{
				bxSel = boxes[i];
			}
		}
...
{	
	...
	if(shape.alturaExtra > 0)
	{
		var img=document.getElementById("altura_extra");
		if (img != null)
			context.drawImage(img,0,0);
	}
}
...
function DrawText(context, shape)
{
	...
	if ((shape.forma==6) || (!EsPlantilla() && (shape.text != "") && (shape.castellerid < 0)))
	{
		...
		if((str.indexOf("SUM(")>-1) && (str.indexOf(")")>4)  && (str.indexOf(";")>4))
		{
			...
			{
				var ind = GetIndexById(ToInt(ar[i]), shape.castellId);
				if (ind > -1)
				{
					suma=suma + ToInt(boxes[ind].altura) + ToInt(boxes[ind].alturaExtra);
				}
			}
			str = suma;
		}
	...
	{	
		...
		else if (shape.malnom != "")
		{
			...
			{
				mides = (ToInt(shape.altura) + ToInt(shape.alturaExtra));// + "  (" + shape.forca + ")";
			}
