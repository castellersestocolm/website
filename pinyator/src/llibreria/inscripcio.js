...
{	
    if (event_id > 0)
	{			
		var count = ToInt(document.getElementById("AE"+event_id+"C"+casteller_id).innerHTML)+1;
		var allowed = true;
		var i=0;
		var trobat = false;
...
{	
    if (event_id > 0)
	{		
		var count = ToInt(document.getElementById("AE"+event_id+"C"+casteller_id).innerHTML);
		if (count > 0)
		{
		...
		xmlhttp.onreadystatechange = function() 
		{
			if (this.readyState == 4 && this.status == 200) 
			{
				...
				if (str == "")
				{
					...
					var operador = count - ToInt(document.getElementById(elementId).innerHTML);
...
function ModificaSom(event_id, operador)
{
	if ((event_id > 0) && (operador != 0))
	{
		...
		{
			if (ToInt(frameList[i].innerHTML) > 0)
			{
				valor=ToInt(frameList[i].innerHTML)+operador;
			}
			else
			{
				valor = 1;
