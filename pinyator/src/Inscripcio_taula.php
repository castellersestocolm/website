...
{	
	...
	if (mysqli_num_rows($result2) > 0) 
	{
		...
		while($row2 = mysqli_fetch_assoc($result2)) 
		{
			...
			$EventMusic = "";
			if (($EsMusic==1) && ($row2["TIPUS"]==-2))
			{
				$EventMusic = "<img src='icons/NotesMusicals.png' width=20 height=30></img>";
			}
			...
			if ($row2["PUBLIC"]>0)
			{	
				$eventNom = "<a href='Actuacio.php?id=".$row2["EVENT_ID"]."&nom=".$malnomPrincipal."'><b>".$row2["EVENT_NOM"]." ".$str_max."</b></a>";
			}
			else
			{
			...
			echo "<tr>			
			<td width='85%'>".$tInici.$EventMusic.$comment.$eventNom."<br>".$row2["DATA"]."<br><p style='font-size:14px'>".$row2["OBSERVACIONS"]."</p>".$acompanyants.$tFinal."</td>";
			if ($visualitzarPenya)
			{
