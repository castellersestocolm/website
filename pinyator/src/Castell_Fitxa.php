...
?>
	<div  class="sidenav" id="navlateral">
		...
		Mides 
		<label class="switch">text
		  <input type="checkbox" id="MostraMides" onclick="Mides()">
		  <span class="slider round"></span>
		</label>
		Popup 
		<label class="switch">text
		  <input type="checkbox" id="MostraPopup" onclick="MostraPopup()" checked>
		  <span class="slider round"></span>
		</label>
		<br>
		Tota la llista		
	...
	<?php
        ...
        $sql_arxivat_extra = "";
        if($dataArxivat != "") {
            $sql_arxivat_extra = " AND ERR.DATA>='".$dataArxivat."'";
        }
		...
		if (mysqli_num_rows($result) > 0) 
		{
			...
			while($row = mysqli_fetch_assoc($result)) 
			{
				...
				$onClick = " onClick='SetCasteller(this)' altura=".$row["ALTURA"]." peu=".$row["PORTAR_PEU"]." lesionat=".$row["LESIONAT"]." camisa=".$row["CAMISA"]." novell=".$row["NOVELL"]." ";
	...
	<div class="canvascontent" style="margin-left: 204px;">
		...
		?>
				...
				<td>
					...
					<div class="popupcasella" id="floatdiv" style="top:200px;left:200px">
						<div class="popupheader" id="floatdivheader">
							<a id="popupheaderCaption" href="Casteller_Fitxa.php?id=" target="_blank"></a>
							<button class="boto_petit boto_remove" id="ClosePopup" onClick="HidePopupCasella()">X</button>
						</div>
						Alça: <input class="text" type="number" id="popupAlturaExtra" min="0" style="width:60px" onchange="CanviAlturaExtra(this.value)" onfocus="focusAlturaExtra()" onblur="focusAlturaExtra()">
						<br>Lesió: <label class="switch">texte
									<input type="checkbox" id="lesionatpopup" value=1 onchange="CanviLesionat()">
									<span class="slider round"></span>
								</label>
						<br>Porta peu: <label class="switch">texte
...
<?php
	...
	{		
		...
		IFNULL(CP.ALTURA_EXTRA, 0) AS ALTURA_EXTRA,
		(SELECT MAX(CORDO) 
...
<script>
<?php
	echo 'window.document.title = "'.$titol.'";';
?>
/* Drag and Drop MOUSE */
...
/* Drag and Drop TOUCH */
window.onload = function() { MostraPopup() }
</script>
</body>
</html>