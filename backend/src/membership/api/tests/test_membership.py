...
@pytest.mark.django_db
class TestGetRenewOptions(NumOperationsMixin, TestCase):
    @classmethod
    def setUpTestData(cls):
        super().setUpTestData()
    ...
    def test_get_options__active_and_processing(self, *args, **kwargs):
        with self.assertNumOperations(num=0, num_selects=1):
            membership_renew_options = get_renew_options(
                user_id=self.user_member_1_obj.id,
            )
        ...
        self.assertEqual(membership_renew_option_towers_modules[0].module, Module.ORG)
        self.assertEqual(
